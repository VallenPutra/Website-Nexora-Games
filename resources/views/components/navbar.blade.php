@php
    // On the homepage these should just be in-page anchors (#games).
    // On any other page (e.g. /contact) they need to link back to the
    // homepage first, then jump to the section (route('home').'#games').
...
<nav x-data="{ open: false }" class="sticky top-0 z-50 border-b-2 border-navy bg-cream">
    <div class="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        ...
        <div class="hidden items-center gap-8 font-sans font-semibold text-navy md:flex">
            <a href="{{ $homeAnchor }}#games" class="hover:text-orange-dark">Games</a>
            <a href="{{ $homeAnchor }}#studio" class="hover:text-orange-dark">Studio</a>
            <a href="{{ $homeAnchor }}#devlog" class="hover:text-orange-dark">Devlog</a>
            <a href="{{ route('contact') }}" class="hover:text-orange-dark">Contact</a>
            <a href="{{ route('login') }}" class="hover:text-orange-dark">Login</a>
        </div>

        <div class="flex items-center gap-3">
            <a href="{{ $homeAnchor }}#games" class="btn-primary hidden sm:inline-flex">Play With Us</a>
    ...
    <div x-show="open" x-cloak class="border-t-2 border-navy bg-cream px-6 py-4 md:hidden">
        <div class="flex flex-col gap-4 font-sans font-semibold text-navy">
            <a href="{{ $homeAnchor }}#games" @click="open = false">Games</a>
            <a href="{{ $homeAnchor }}#studio" @click="open = false">Studio</a>
            <a href="{{ $homeAnchor }}#devlog" @click="open = false">Devlog</a>
            <a href="{{ route('contact') }}" @click="open = false">Contact</a>
            <a href="{{ route('login') }}" @click="open = false">Login</a>
            <a href="{{ $homeAnchor }}#games" class="btn-primary w-fit">Play With Us</a>
        </div>
    </div>
</nav>